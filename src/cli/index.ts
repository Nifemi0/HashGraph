...
program
  .name("hashgraph")
  .description("HashGraph CLI for compiling and analyzing smart contracts")
  .version("1.0.0");
  ...
  .action(async (address, options) => {
      try {
        ...
        if (options.export) {
            ...
            fs.writeFileSync(path.join(options.export, 'hashgraph.json'), JSON.stringify(graph, null, 2));
            ...
            fs.writeFileSync(path.join(options.export, 'hashgraph.md'), md);
