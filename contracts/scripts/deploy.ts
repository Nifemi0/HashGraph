...
async function main() {
  ...
  const Registry = await ethers.getContractFactory("HashGraphRegistry");
  ...
  console.log("\n✅ HashGraphRegistry deployed:", addr);
  ...
  const fileName = networkName === 'hashkey' ? 'mainnet.json' : `${networkName}.json`;
