 ...
 * and assert structural properties of the resulting HashGraphSchema.
 *
...
import type { HashGraphSchema } from "../../src/types/schema";
...
describe("Protocol Graph — integration", () => {
  ...
  describe.each(FIXTURES)("$name — shared invariants", (fx) => {
    ...
    it("registry block is present", () => {
      ...
      expect(graph.registry!.deploymentNetwork).toBe("HashKey Mainnet");
    });
  });
