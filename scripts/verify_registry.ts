/**
 * Submit HashGraphRegistry source for verification on HashKey Blockscout.
...
const API = "https://hsk.blockscout.com/api";
...
const SOURCE_PATH = path.join(
  __dirname,
  "../contracts/contracts/HashGraphRegistry.sol"
);
...
function loadStandardInput(): string {
  ...
  return JSON.stringify({
    language: "Solidity",
    sources: { "contracts/HashGraphRegistry.sol": { content: source } },
    settings: {
      optimizer: { enabled: true, runs: 200 },
      evmVersion: "paris",
      outputSelection: {
...
async function verify() {
  ...
  const body = new URLSearchParams({
    ...
    contractname: "contracts/HashGraphRegistry.sol:HashGraphRegistry",
  ...
  if (data.status !== "1") {
    ...
    console.error(
      `https://hsk.blockscout.com/address/${ADDRESS}/contract-verification`
    );
    process.exitCode = 1;
    return;
  }
