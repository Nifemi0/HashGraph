# HashGraph Programmatic SDK Guide

The `@hashgraph/mcp` package exports the `HashGraphClient` class, allowing you to programmatically access all the compiler pipeline stages, semantic reasoning, transaction simulations, and decoding layers inside your Node.js/TypeScript applications.
...
```bash
# RPC URL for HashKey Chain (Defaults to HashKey Mainnet)
...
async function main() {
  const address = "0xF1B50eD67A9e2CC94Ad3c477779E2d4cBfFf9029"; // Sample Contract
...
The `HashGraphClient` class exposes the following methods:

### `getProtocolGraph(address: string, forceRefresh?: boolean): Promise<HashGraphSchema>`
...
* **Returns:** A `HashGraphSchema` object containing structural details, semantic intent, security guardrails, and developer integration notes.
...
async function inspectPermissions(contracts: string[]) {
  for (const address of contracts) {
    ...
    graph.security.privileged_functions.forEach(func => {
      console.log(`  - ${func.name}() allowed for: ${func.roles.join(", ")}`);
    });
  }
}
```
